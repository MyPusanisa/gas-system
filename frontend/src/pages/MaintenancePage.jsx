...
const fetchMaintenances = async () => {
    try {
      ...
      try {
        ...
        console.warn("get_maintenance parse error:", text);
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const fetchCylinders = async () => {
    try {
      ...
      try {
        ...
        console.error("JSON Parse Error in get_due_cylinders:", text);
      }
    } catch (err) {
      console.error(err);
    } finally {
          ...
          <tbody>
            {latestRecords.length > 0 ? (
              latestRecords.map((item, index) => {
                // เช็กถ้าเป็น 0 หรือว่าง ให้เปลี่ยนเป็น "-"
                const sn = item.serial_number;
                const cid = item.cylinder_id;
                let displaySerial = "-";

                if (sn && sn !== "0" && sn !== 0) {
                  displaySerial = sn;
                } else if (cid && cid !== "0" && cid !== 0) {
                  displaySerial = cid;
                }
